...
all five HERO Producer-A vocabularies.
...
It also adds **reachability pins** for the three vocabularies whose
derive-helpers we own (`HERO_BIAS`, `HERO_RISK`, `HERO_TRUST`,
`HERO_ACTION`): every vocab member must be returned from at least one
branch of the corresponding helper. Prevents dead vocab entries.

The two passthrough vocabularies (:data:`HERO_MARKET_MODE_VOCAB` and
