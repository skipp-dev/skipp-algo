...
class TestConstantTimeTokenCompare:
    """C1: _ct_eq compares fixed-width normalized buffers so token length is
    ...
    def test_compares_equal_length_buffers(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert seen[0][0] == seen[0][1], "compare buffers must be same fixed width"
        assert seen[0][0] >= 16_384

    def test_rejects_multibyte_input_at_inclusive_byte_cap(self) -> None:
        import services.live_overlay_daemon.main as main_mod

        # 4096 code points are at the character cap, and 16,384 bytes in UTF-8.
        # The byte cap is inclusive, so this boundary input is rejected.
        attacker_value = "😀" * 4096
