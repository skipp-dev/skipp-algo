...
def test_micro_profile_row_encodes_modifier_presence_inline() -> None:
    ...
    assert 'int micro_profile_encoded = int(math.round(nz(src_micro_profile_code, 0.0)))' in dashboard_source
    assert 'int micro_profile_row_code = micro_profile_row_code_from_code(micro_profile_encoded)' in dashboard_source
    assert 'dashboard_row(smc_dashboard, 67, "Micro Profile", decode_micro_profile_text(micro_profile_row_code), status_bg(row_status(micro_profile_row_code)), txt)' in dashboard_source
...
def test_hard_gate_decoders_reproduce_current_bus_v2_contract() -> None:
    ...
    assert 'dashboard_row(smc_dashboard, 35, "Session", decode_session_text(session_row_code)' in dashboard_source
    assert 'dashboard_row(smc_dashboard, 36, "Market Gate", decode_market_text(market_row_code)' in dashboard_source
    assert 'dashboard_row(smc_dashboard, 37, "Vola Regime", decode_vola_text(vola_row_code)' in dashboard_source
    assert 'dashboard_row(smc_dashboard, 38, "Micro Session", decode_micro_session_text(micro_session_row_code)' in dashboard_source
    assert 'dashboard_row(smc_dashboard, 39, "Micro Fresh", decode_micro_fresh_text(micro_fresh_row_code, freshness_code, source_state_code)' in dashboard_source
    assert 'dashboard_row(smc_dashboard, 40, "Volume Data", decode_volume_data_text(volume_data_row_code)' in dashboard_source
...
def test_quality_score_uses_fixed_local_bounds_text() -> None:
    ...
    assert 'dashboard_row(smc_dashboard, 51, "Primary | Signal + Ensemble", quality_stack_text(src_quality_score, lean_transport_row_code), primary_metric_bg(row_status(quality_score_row_code)), txt)' in dashboard_source
...
def test_lean_transport_row_exposes_ensemble_and_library_volatility() -> None:
    ...
    assert 'dashboard_row(smc_dashboard, 56, "Vol Regime | Lib", decode_vol_regime_stack_text(vol_regime_row_code, lean_transport_row_code), status_bg(row_status(vol_regime_row_code)), txt)' in dashboard_source
