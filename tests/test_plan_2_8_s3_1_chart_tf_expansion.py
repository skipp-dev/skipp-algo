...
Guards the expansion from {15m, 1H} to {5m, 15m, 1H, 4H} for:
...
def test_release_reference_tuple_contains_the_four_tfs() -> None:
    assert RELEASE_REFERENCE_TIMEFRAMES == ("5m", "15m", "1H", "4H")


def test_benchmark_cli_default_covers_all_four_tfs() -> None:
    parser = build_parser()
    ns = parser.parse_args([])
    tfs = [tf.strip() for tf in ns.timeframes.split(",")]
    assert tfs == ["5m", "15m", "1H", "4H"]


def test_rolling_workflow_default_timeframes_covers_all_four() -> None:
    wf = yaml.safe_load(ROLLING_WF.read_text(encoding="utf-8"))
    inputs = wf.get("on", wf.get(True))["workflow_dispatch"]["inputs"]
    assert inputs["timeframes"]["default"] == "5m,15m,1H,4H"


def test_rolling_workflow_run_step_fallback_covers_all_four() -> None:
    text = ROLLING_WF.read_text(encoding="utf-8")
    # The shell fallback when inputs.timeframes is empty must point at
    # the new 4-TF default, not the old 2-TF default.
    assert "|| '5m,15m,1H,4H'" in text
