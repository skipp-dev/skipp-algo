...
def test_databento_volatility_cache_is_warmed_across_runs() -> None:
    ...
    # window-end and the restore-keys fallback MUST be shard-scoped so a
    # new day inherits the prior day's cache for delta-fetch.
    text = _read()
    ...
    assert (
        "key: dbnv-cache-shard-${{ matrix.shard_id }}-of-${{ matrix.shard_of }}-${{ matrix.end_date }}"
        in step_block
    )
    assert "restore-keys:" in step_block
    assert "dbnv-cache-shard-${{ matrix.shard_id }}-of-${{ matrix.shard_of }}-" in step_block
