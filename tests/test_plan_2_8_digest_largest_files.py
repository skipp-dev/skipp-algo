"""Tests for ``scripts/plan_2_8_digest_largest_files.py``."""
...
SCRIPT = REPO / "scripts" / "plan_2_8_digest_largest_files.py"
WEEKLY = REPO / ".github" / "workflows" / "plan-2-8-weekly-digest.yml"


def _load():
    spec = importlib.util.spec_from_file_location(
        "plan_2_8_digest_largest_files", SCRIPT,
    )
    assert spec and spec.loader
    mod = importlib.util.module_from_spec(spec)
    sys.modules["plan_2_8_digest_largest_files"] = mod
...
def test_empty_dir(tmp_path: Path) -> None:
    rep = lf.build(tmp_path, 10)
    assert rep["count"] == 0


def test_descending_order(tmp_path: Path) -> None:
...
def test_zero_top_n_returns_all(tmp_path: Path) -> None:
    (tmp_path / "a").write_bytes(b"x")
    (tmp_path / "b").write_bytes(b"xx")
    rep = lf.build(tmp_path, 0)
    assert rep["count"] == 2


def test_markdown_shape(tmp_path: Path) -> None:
    (tmp_path / "a").write_bytes(b"x")
    md = lf.render_markdown(lf.build(tmp_path, 10))
    assert "largest files" in md
...
def test_cli_json(tmp_path: Path) -> None:
    ...
    out = tmp_path / "o.json"
    rc = lf.main([
        "--artifact-dir", str(tmp_path), "--top-n", "5",
...
def test_weekly_has_largest_files_steps() -> None:
    pytest.importorskip("yaml")
    wf = _wf(WEEKLY)
    steps = wf["jobs"]["weekly-digest"]["steps"]
    names = [s.get("name", "") for s in steps]
    assert "Plan 2.8 digest largest files" in names
    assert "Upload Plan 2.8 digest largest files" in names
    step = next(s for s in steps
                if s.get("name") == "Plan 2.8 digest largest files")
    assert "plan_2_8_digest_largest_files.py" in step["run"]
