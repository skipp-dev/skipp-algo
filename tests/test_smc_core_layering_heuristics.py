...
def test_bullish_heat_strengthens_bullish_ob_fvg() -> None:
    ...
    layered = apply_layering(structure, _meta(tech_strength=0.9, tech_bias="BULLISH", news_strength=0.4, news_bias="BULLISH"))

    ob_style = layered.zone_styles["ob:1"]
    fvg_style = layered.zone_styles["fvg:1"]
...
def test_bearish_heat_strengthens_bearish_ob_fvg() -> None:
    ...
    layered = apply_layering(structure, _meta(tech_strength=0.85, tech_bias="BEARISH", news_strength=0.4, news_bias="BEARISH"))

    ob_style = layered.zone_styles["ob:2"]
    fvg_style = layered.zone_styles["fvg:2"]
...
def test_counter_heat_discourages_zone() -> None:
    ...
    layered = apply_layering(structure, _meta(tech_strength=0.8, tech_bias="BEARISH", news_strength=0.3, news_bias="BEARISH"))

    style = layered.zone_styles["ob:3"]
...
def test_invalid_ob_and_fvg_are_dimmed() -> None:
    ...
    layered = apply_layering(structure, _meta())

    ob_style = layered.zone_styles["ob:4"]
    fvg_style = layered.zone_styles["fvg:4"]
...
def test_holiday_suspect_blocks_everything() -> None:
    ...
    layered = apply_layering(structure, _meta(regime="HOLIDAY_SUSPECT"))

    for style in layered.zone_styles.values():
...
def test_low_volume_dims_and_discourages() -> None:
    ...
    layered = apply_layering(structure, _meta(regime="LOW_VOLUME", tech_strength=0.9, tech_bias="BULLISH"))

    style = layered.zone_styles["ob:6"]
    assert style.render_state == "DIMMED"
...
def test_sweep_remains_neutral_with_side_reason() -> None:
    ...
    layered = apply_layering(structure, _meta(tech_strength=0.9, tech_bias="BULLISH", news_strength=0.9, news_bias="BULLISH"))

    buy_style = layered.zone_styles["sweep:buy"]
    sell_style = layered.zone_styles["sweep:sell"]
...
def test_bos_choch_have_marker_reasons() -> None:
    ...
    assert "BOS" in layered.zone_styles["bos:6"].reason_codes
    assert "CHOCH" in layered.zone_styles["choch:6"].reason_codes
...
def test_reason_codes_are_deduplicated() -> None:
    ...
    layered = apply_layering(structure, _meta(regime="NORMAL", tech_strength=0.9, tech_bias="BULLISH", news_strength=0.9, news_bias="BULLISH"))

    reasons = layered.zone_styles["ob:7"].reason_codes
