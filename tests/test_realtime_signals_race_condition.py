...
def test_poll_once_does_not_hold_active_signals_lock(tmp_path: Path, monkeypatch: Any) -> None:
    ...
    # After fix: poll_once acquires _lock for every read AND write of
    # _active_signals. We expect at least 3 acquire/release pairs:
    # 1) Expiry prune, 2) VisiData row snapshot, 3) Re-qualification + final payload.
    acquire_count = lock_events.count("acquire")
    assert acquire_count >= 3, (
        f"poll_once acquired _lock only {acquire_count} time(s); "
        f"expected >=3 to protect all _active_signals accesses"
    )
