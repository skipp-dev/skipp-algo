...
def _export_lines(text: str) -> list[str]:
    return [line for line in text.splitlines() if line.startswith("export const")]


def _export_names(text: str) -> set[str]:
    names: set[str] = set()
    for line in _export_lines(text):
...
V5_FIELD_INVENTORY = sorted(_export_names(_GENERATED_PINE_PATH.read_text(encoding="utf-8")))
...
class TestFullEnrichment:
    ...
    def test_volume_regime(self, base_csv: Path, tmp_path: Path):
        text = _run_pipeline(base_csv, tmp_path, enrichment=_full_enrichment())
        match = re.search(r'VOLUME_LOW_TICKERS = "([^"]*)"', text)
        assert match is not None
        assert "TSLA" in match.group(1)
...
class TestV5FieldInventory:
    ...
    def test_no_unexpected_export_fields(self, base_csv: Path, tmp_path: Path):
        text = _run_pipeline(base_csv, tmp_path, enrichment=_full_enrichment())
        found_names = _export_names(text)
...
class TestManifestEnrichment:
    def test_manifest_contains_library_field_version(self, base_csv: Path, tmp_path: Path):
        ...
        assert manifest.get("library_field_version") == "v8.0a"
