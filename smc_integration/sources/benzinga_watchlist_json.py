...
BENZINGA_WATCHLIST_JSON = Path(__file__).resolve().parents[2] / "reports" / "benzinga_watchlist_snapshot.json"
...
def describe_source() -> SourceDescriptor:
    return SourceDescriptor(
        name="benzinga_watchlist_json",
        path_hint="reports/benzinga_watchlist_snapshot.json",
        ...
        notes=[
            "Benzinga watchlist snapshot source with symbol-level context.",
...
def _load_payload() -> dict[str, Any]:
    if not BENZINGA_WATCHLIST_JSON.exists():
        raise FileNotFoundError(f"benzinga source not found: {BENZINGA_WATCHLIST_JSON}")
    payload = json.loads(BENZINGA_WATCHLIST_JSON.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError(f"benzinga payload must be an object: {BENZINGA_WATCHLIST_JSON}")
    return payload
...
def _extract_rows(payload: dict[str, Any]) -> list[dict[str, Any]]:
    for key in ("symbols", "watchlist", "items", "data"):
        ...
        if isinstance(rows, list):
            ...
            if out:
                return out
    raise ValueError("benzinga payload has no symbol rows")
...
def _extract_news(row: dict[str, Any], *, fallback_asof_ts: float) -> tuple[dict[str, Any] | None, str | None]:
    direct = row.get("news")
    direct_map = direct if isinstance(direct, dict) else {}

    strength = _coerce_optional_float(direct_map.get("strength"))
    if strength is None:
        strength = _coerce_optional_float(row.get("news_strength"))
    ...
    if asof_ts is None:
        asof_ts = _coerce_optional_float(row.get("news_asof_ts"))
    if asof_ts is None:
        asof_ts = fallback_asof_ts

    stale = _coerce_optional_bool(direct_map.get("stale"))
    if stale is None:
        stale = _coerce_optional_bool(row.get("news_stale"))
    if stale is None:
        stale = False
...
def _select_symbol_row(payload: dict[str, Any], symbol: str) -> dict[str, Any]:
    ...
    if not matching:
        raise ValueError(f"symbol {wanted} not present in benzinga source")
...
def load_raw_meta_input(symbol: str, timeframe: str) -> dict[str, Any]:
    ...
    if asof_ts_opt is not None:
        ...
        raise ValueError("benzinga row is missing both asof_ts and trade_date")
    ...
    payload = {
        ...
        "provenance": [
            "repo:reports/benzinga_watchlist_snapshot.json",
            f"repo:reports/benzinga_watchlist_snapshot.json#symbol={resolved_symbol}",
            "smc_integration:partial_structure_only",
        ],
    }

    news, news_status = _extract_news(row, fallback_asof_ts=asof_ts)
    if news is not None:
        payload["news"] = news
        payload["provenance"].append("smc_integration:news_mapped_from_benzinga")
    elif news_status:
        payload[_META_DOMAIN_STATUS_KEY] = {"news": news_status}
