...
class Config:
    ...
    # ── Polling cadence ─────────────────────────────────────────
    poll_interval_s: float = field(default_factory=lambda: float(os.getenv("POLL_INTERVAL_S", "2.0")))

    # ── FMP endpoints (stable) ──────────────────────────────────
    stock_latest_page: int = field(default_factory=lambda: int(os.getenv("FMP_STOCK_LATEST_PAGE", "0")))
    stock_latest_limit: int = field(default_factory=lambda: int(os.getenv("FMP_STOCK_LATEST_LIMIT", "200")))
    press_latest_page: int = field(default_factory=lambda: int(os.getenv("FMP_PRESS_LATEST_PAGE", "0")))
    press_latest_limit: int = field(default_factory=lambda: int(os.getenv("FMP_PRESS_LATEST_LIMIT", "50")))

    # ── Benzinga REST settings ──────────────────────────────────
    benzinga_rest_page_size: int = field(default_factory=lambda: int(os.getenv("BENZINGA_REST_PAGE_SIZE", "100")))
    ...
    # ── Retention ───────────────────────────────────────────────
    keep_seen_seconds: float = field(default_factory=lambda: float(os.getenv("KEEP_SEEN_SECONDS", str(2 * 86400))))
    keep_clusters_seconds: float = field(default_factory=lambda: float(os.getenv("KEEP_CLUSTERS_SECONDS", str(2 * 3600))))
