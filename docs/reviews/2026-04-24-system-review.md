...
10. **P-1** (Provider — Benzinga Quantified News, COST/ENTITLEMENT): Endpoint ist `/api/v2/newsquantified` (ein Wort, **Authorization-Header** statt `?token=`, snake_case Params per [offizieller Doku](https://docs.benzinga.com/api-reference/newsquantified-api/get-newsquantified-data)). Code-Pfad in `newsstack_fmp/ingest_benzinga.py:fetch_benzinga_quantified_news` korrigiert (PR-D11, 2026-04-30). Live-Probe gegen Benzinga: HTTP 401 "Access denied for user 0 anonymous" → Endpoint erreichbar + Auth-Format korrekt, aber unser API-Key hat **keine Entitlement** (Premium-Tier). Pipeline weiterhin grün via `mark_endpoint_disabled` in `newsstack_fmp/_bz_http.py:log_fetch_warning`. **Wert wenn aktiviert:** Benzinga-vorberechnete News→Price-Reaction-Metriken (DayOpen, OpenGap%, Range%, 1_Minute% bis 120_Minutes%, ShortInterest%) — würde die "Reaction-Heuristics" durch Provider-ground-truth ersetzen statt eigener Schätzung in `terminal_reaction_state.py` / Catalyst-Score. **Aktion:** Plan-Upgrade vs. Eigenberechnung-Beibehaltung wirtschaftlich abwägen; nicht eilig (Pipeline läuft korrekt ohne).
