...
10. **P-1** (Provider — Benzinga Quantified News, COST/ENTITLEMENT): Endpoint ist `/api/v2/newsquantified` (ein Wort, snake_case Params, Auth via `?token=` wie der Rest unserer Benzinga-Integration und der offizielle [`benzinga-python-client`](https://github.com/Benzinga/benzinga-python-client/blob/master/benzinga/news_data.py)). Code-Pfad in `newsstack_fmp/ingest_benzinga.py:fetch_benzinga_quantified_news` korrigiert (PR-D11 + Audit-Korrektur PR-D12, 2026-04-30). Live-Probe heute: HTTP 401 *"Access denied for user 0 anonymous"* mit BEIDEN Auth-Methoden (`?token=` und `Authorization`) — Endpoint und Auth sind also korrekt, aber unser API-Key hat **keine Entitlement** für dieses Premium-Add-on. Verifikation: derselbe Key liefert HTTP 200 auf `/api/v2/news` und `/api/v2/channels`. Pipeline weiterhin grün via `mark_endpoint_disabled` in `newsstack_fmp/_bz_http.py:log_fetch_warning`. **Wert wenn aktiviert:** Benzinga-vorberechnete News→Price-Reaction-Metriken (DayOpen, OpenGap%, Range%, 1_Minute% bis 120_Minutes%, ShortInterest%) — würde die Reaction-Heuristics durch Provider-ground-truth ersetzen statt eigener Schätzung in `terminal_reaction_state.py` / Catalyst-Score. **Aktion:** Im Benzinga Account-Dashboard unter "Subscriptions / Add-ons" nach "Newsquantified" suchen oder Benzinga Sales kontaktieren — der Key selbst ist gültig, Newsquantified ist ein separates Produkt-SKU.
...
12. **P-3** (Provider — Benzinga Financial v2/v2.1, DEAD-PATHS): Audit am 2026-04-30 deckte 6 retired Endpoints in `newsstack_fmp/ingest_benzinga_financial.py` + `ingest_benzinga_calendar.py` auf, alle liefern HTTP 404 *"Page Not Found / no Route matched"*: `/api/v2/search`, `/api/v2/security`, `/api/v2/tickerDetail`, `/api/v2.1/instruments`, `/api/v2.1/calendar/options_activity`, `/api/v2.1/ownership`. **HOCH-PRIO:** `options_activity` wird aktiv aus `open_prep/streamlit_monitor.py:178` und `terminal_poller.py:57` aufgerufen — bricht still durch `mark_endpoint_disabled`, Output ist seit dem Server-Side-Retire leer. Restliche 5 sind tote Read-Methoden. **Aktion:** entweder neue Pfade in der aktuellen Benzinga API recherchieren (Options-Activity wurde vermutlich umbenannt) oder die Funktionen + Konstanten ersatzlos entfernen. Verifikation gleicher Key, gleicher Token: `/api/v2.1/calendar/ratings` und `/api/v2.1/calendar/earnings` liefern HTTP 200, also kein Auth-Problem.
13. **P-4** (Provider — FMP `/stable/insider-trading` + Holders, RENAMES/DEAD-PATHS): Live-Audit 2026-04-30: `/stable/insider-trading` retired (404), Ersatz `/stable/insider-trading/latest` und `/stable/insider-trading/search` funktionieren ✅ (bereits in `open_prep/macro.py:1254` als Kommentar dokumentiert, aber nicht im Code propagiert). Zusätzlich retired: `/stable/etf-holder/{symbol}` und `/stable/institutional-holder/{symbol}`. `/stable/etf/holdings` existiert aber liefert leer — wahrscheinlich verschachteltes Schema. `/stable/institutional-ownership/symbol-positions-summary` funktioniert mit `year=` Param. **Aktion:** sweep über `newsstack_fmp/ingest_fmp.py` für insider-trading-Aufrufer + Decision Holder-Migration vs. Cleanup.
14. **P-5** (Provider — FMP `/stable/analyst-estimates` PARAMETRISIERUNG): liefert HTTP 400 ohne `period=annual|quarterly` Query-Param. Aktueller Code übergibt den Param nicht → silent empty. **Aktion:** kleiner Fix, Default `period=annual` setzen.
15. **P-6** (Provider — FMP `/stable/fear-and-greed-index` DEAD-CODE-CLEANUP, LOW): Live-Audit am 2026-04-30 zeigte HTTP 404 ohne FMP-Replacement (auch legacy `/api/v3/fear-and-greed-index` antwortet 403 *"Legacy Endpoint"*). **Bereits mitigiert in beiden Konsumenten:** (a) Crypto F&G für die Bitcoin-Kachel kommt primär aus `https://api.alternative.me/fng/` mit FMP nur als Fallback (`terminal_bitcoin.py:633-678`), (b) Equity F&G für Open-Prep / Regime kommt aus CNN über `open_prep/sentiment_fng.py:fetch_cnn_equity_fear_greed` (eingebunden in `open_prep/run_open_prep.py:50` und `open_prep/regime.py:54`). Der Caller `open_prep/macro.py:998 get_fear_and_greed_index` ist somit toter Code, gibt nur noch `[]` über `_warn_once` zurück. **Aktion:** Methode + zugehörigen Test in `tests/test_macro_extended_coverage.py:760` entfernen; FMP-Fallback-Pfad in `terminal_bitcoin.py:680-693` ebenfalls entfernen (alternative.me ist verlässlich genug, Fallback ist toter Branch).
16. **P-7** (Provider — NewsAPI.ai/Event Registry, INFO/PERF): Vollständiger Audit am 2026-04-30 gegen [Event Registry Doku](https://eventregistry.org/documentation): alle drei aktiv genutzten Endpoints `article/getArticles`, `event/getEvents`, `minuteStreamArticles` liefern HTTP 200 mit erwarteten JSON-Shapes (`articles.results[*]`, `events.results[*]`, Top-Level Listen). Kein Doku-Drift, kein Pfad-Bug. **Performance-Beobachtung:** `getArticles` braucht bei breiten Keywords (z.B. `"Apple"` mit ~9M Treffern) bis zu **24 s round-trip** — `scripts/smc_newsapi_ai.py` setzt einen 30-s `httpx`-Timeout, also OK aber wenig Marge. Edge-Case bei sehr breiten Suchen → Pollers könnten sporadisch timeouten. **Aktion:** (a) Default-Timeout in `_request_payload` (~L242) auf 45 s erhöhen, (b) Default-`articlesCount` reduzieren oder Keyword-Bounding (Datum + Sprache) erzwingen, (c) Weiterhin gilt: legacy stub `terminal_newsapi.py` bleibt deprecated — siehe L-1.
17. **P-INFO Audit-Bestätigung 2026-04-30**: Provider-Endpoint-Sweep deckt jetzt 41/45 produktiv genutzten FMP `/stable/*`-Pfaden + 3/3 Event-Registry-Endpoints + alle aktiven Benzinga `/api/v2/*` und `/api/v2.1/*`-Pfade ab. Verifiziert via Live-Probe gegen reale API-Keys. Keine still abdriftenden Response-Schemas. Liste der bestätigten OK-Pfade liegt in `/memories/repo/{benzinga,fmp,eventregistry}-endpoint-paths.md`.
18. **P-8** (Provider — IBKR Library-Maintenance, **HIGH/MEDIUM**): `ib_insync` letztes PyPI-Release **v0.9.86 vom 2023-07-02** (Stand 2026-04-30 — fast 3 Jahre alt). Original-Maintainer Ewald de Wit ist [verstorben](https://groups.io/g/insync/topic/passing_of_ewald_de_wit/97993770), Repo `erdewit/ib_insync` archiviert. Community-Fork `ib_async` (org `ib-api-reloaded`, Repo `github.com/ib-api-reloaded/ib_async`) ist **aktiv gepflegt**: v2.1.0 am 2025-12-08, v2.0.1 am 2025-06-22, mehrere 1.0.x-Releases in 2024 — drop-in API-kompatibel (`from ib_async import IB, LimitOrder, MarketOrder, Stock, Order` ersetzt 1:1 `from ib_insync import …`, dito `ib_async.objects.WshEventData`). **Risiko des Status-quo:** keine Bugfixes mehr, kein Support für neuere TWS-API-Versionen, kein Security-Tracking. Aktuell läuft die Order-Execution-Pipeline (`scripts/execute_ibkr_watchlist.py`, `scripts/imbalance_data.py`, `scripts/wsh_earnings_calendar.py`, `scripts/collect_opening_imbalances.py`, `scripts/smoke_smc_to_ibkr_adapter.py`, `scripts/run_ibkr_open_execution.py`, `scripts/ib_client_id.py`) und alle IBKR-Tests gegen die stale Lib. **Aktion:** Migrations-PR — alle `from ib_insync …` → `from ib_async …` ersetzen, `requirements.txt` swap (`ib_insync` → `ib_async>=2.1.0`), Test-Mock-Patches anpassen (`monkeypatch.setattr("scripts.execute_ibkr_watchlist.ib_insync", …)` → `ib_async`). Keine Logik-Änderungen erwartet (drop-in).
19. **P-9** (Provider — TradingView TA-Library, LOW): `tradingview_ta` letztes PyPI-Release **v3.3.0 vom 2022-10-05** (Stand 2026-04-30 — 3,5 Jahre alt). Repo `brian-the-dev/python-tradingview-ta` zeigt minimale Aktivität. **Endpoint funktioniert weiter:** Live-Probe am 2026-04-30 zeigt `https://scanner.tradingview.com/america/scan` POST → HTTP 200 mit erwartetem `{data:[{s,d:[…]}], totalCount}`-Shape (~0.32 s rtt). **Bereits mitigiert:** `terminal_technicals.py:295` `_try_fmp_fallback` springt bei TV-Failure auf FMP `/stable/technical-indicators/{rsi,sma,ema,adx,williams}` um. **Aktion:** monitoren; bei zukünftigem Scanner-Schema-Drift ggf. TV-Quelle ganz droppen und FMP-only nutzen — kein akuter Handlungsbedarf.
20. **P-10** (Provider — TradingView Headlines, INFO): Undokumentiertes privates Endpoint `https://news-headlines.tradingview.com/v2/view/headlines/symbol?client=web&lang=en&symbol={EXCHANGE:TICKER}` am 2026-04-30 live geprüft (AAPL/JPM/NVDA): jeweils HTTP 200, ~95-104 KB, ~0.25-0.48 s rtt, **200 Items pro Call**. JSON-Schema unverändert: `[id, permission, provider, published, relatedSymbols, source, sourceLogoId, storyPath, title, urgency]` — entspricht dem Parser in `terminal_tradingview_news.py:_parse_items` 1:1. Provider-Tier-Mapping (`reuters` / `dow-jones` → TIER_1) verifiziert. Eingebauter 1,0 s Rate-Limiter + Browser-User-Agent + 8 s Timeout reichen aus. **Kein Handlungsbedarf.**
21. **P-INFO Audit-Bestätigung 2026-04-30 (Erweiterung TV + IBKR)**: Provider-Sweep deckt jetzt zusätzlich ab — TradingView Headlines (privat, undokumentiert), TradingView Scanner (privat, via `tradingview_ta`), IBKR/TWS-Surface (`reqMktData` mit `genericTickList="225"` für Opening Imbalance, `reqWshMetaData`/`reqWshEventData` mit `WshEventData(country, watchlist, filter_categories, future_days)` Filter, Order-Helpers). TWS-Doku (https://interactivebrokers.github.io/tws-api/) bestätigt das verwendete API-Surface. PyPI-Status der externen Libs ist neuer Audit-Punkt (siehe P-8 + P-9). Verifizierte Endpoint-/Surface-Listen liegen in `/memories/repo/{tradingview,ibkr-tws}-surface.md` (nach Merge dieser PR).
