...
def _is_market_hours_fallback() -> bool:
    ...
    window on weekdays.  Fully guarded — never raises.
    """
...
def _cached_bz_options_op(api_key: str, tickers: str) -> list[dict[str, Any]]:
    ...
    Provider precedence (post-2026-05-12 UW flow-alerts removal):
      1. **Databento OPRA.PILLAR** — enabled when ``ENABLE_OPRA_UOA=1`` and
         a Databento key is configured. Canonical options-flow path.
    ...
    def _render_open_prep_snapshot() -> None:
        ...
        if (
            ...
            try:
                ...
                # Any time: recover after absolute max (60 min)
                _ar_should_recover = (
