...
class FMPClient:
    ...
    def get_splits_calendar(self, date_from: date, date_to: date) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/splits-calendar); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
    ...
    def get_dividends_calendar(self, date_from: date, date_to: date) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/dividends-calendar); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
    ...
    def get_ipos_calendar(self, date_from: date, date_to: date) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/ipos-calendar); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
    ...
    def get_upgrades_downgrades(
        ...
        try:
            ...
            _log_feature_unavailable_once(
                "stable/grades",
                "FMP feature unavailable (stable/grades); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
    ...
    def get_treasury_rates(self, date_from: date | None = None, date_to: date | None = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/treasury-rates); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
    ...
    def get_house_trading(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                "stable/house-latest",
                "FMP feature unavailable (stable/house-latest); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
    ...
    def get_dcf(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/discounted-cash-flow); continuing without data.",
            )
            return {}
        if isinstance(data, dict):
            return dict(data)
    ...
    def get_company_profile(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                "stable/profile",
                "FMP feature unavailable (stable/profile); continuing without data.",
            )
            return {}
        if isinstance(data, dict):
            return dict(data)
    ...
    def get_price_target_consensus(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/price-target-consensus); continuing without data.",
            )
            return {}
        if isinstance(data, dict):
            return dict(data)
    ...
    def get_price_target_summary(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/price-target-summary); continuing without data.",
            )
            return {}
        if isinstance(data, dict):
            return dict(data)
    ...
    def get_grades_consensus(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/grades-consensus); continuing without data.",
            )
            return {}
        if isinstance(data, dict):
            return dict(data)
    ...
    def get_analyst_estimates(self, symbol: str, *, period: str = "quarter", limit: int = 8) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/analyst-estimates); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
    ...
    def get_earnings_report(self, symbol: str, limit: int = 12) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                "stable/earnings",
                "FMP feature unavailable (stable/earnings); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
    ...
    def get_historical_price_eod_full(self, symbol: str, date_from: date, date_to: date) -> list[dict[str, Any]] | dict[str, Any]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                "stable/historical-price-eod/full",
                "FMP feature unavailable (stable/historical-price-eod/full); continuing without data.",
            )
            return []
        if isinstance(data, dict):
            return dict(data)
    ...
    def get_intraday_chart(
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/historical-chart); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
    ...
    def get_index_quote(self, symbol: str = "^VIX") -> dict[str, Any]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                "stable/quote",
                "FMP feature unavailable (stable/quote); continuing without data.",
            )
            return {}
        if isinstance(data, dict):
            return dict(data)
    ...
    def get_sector_performance_snapshot(self, as_of: date | None = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _log_feature_unavailable_once(
                ...
                "FMP feature unavailable (stable/sector-performance-snapshot); continuing without data.",
            )
            return []
        return list(data) if isinstance(data, list) else []
