"""Plan 2.8 ledger first red.

Reports the timestamp of the earliest ``red`` status record
in the ledger (by insertion order). If no red records exist,
``found`` is ``False``.
"""
...
def render_markdown(report: dict[str, Any]) -> str:
    if not report.get("found"):
        return "# Plan 2.8 ledger first red\n\n_none_\n"
    return (
        "# Plan 2.8 ledger first red\n"
        "\n"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Timestamp of the first red ledger record.",
    )
