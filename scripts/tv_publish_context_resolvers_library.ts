...
type PublishContextResolversReport = {
  generatedAt: string;
  ok: boolean;
  contractOk: boolean;
...
function parseArgs(): CliArgs {
  ...
  return {
    ...
    out: path.resolve(
      getFlag(
        "--out",
        `automation/tradingview/reports/publish-context-resolvers-library-${utcNow().replace(/[:.]/g, "-")}.json`,
      ),
    ),
...
function verifyContextResolversPublishContract(cli: CliArgs): ContractDetails {
  if (!fs.existsSync(cli.library)) {
    throw new Error(`Missing context_resolvers library source: ${cli.library}`);
  }
  ...
  if (!libraryText.includes(expectedLibraryHeader)) {
    throw new Error(`Context resolvers library header mismatch: expected ${expectedLibraryHeader}`);
  }
  ...
  if (!Number.isFinite(cli.version) || cli.version < 1) {
    throw new Error(`Context resolvers library version must be a positive integer, received: ${cli.version}`);
  }
...
export async function runPublishContextResolversLibraryCli(): Promise<number> {
  ...
  try {
    details = verifyContextResolversPublishContract(cli);
    const session = await newTradingViewSession();
    try {
      if (!session.authResolution.authReusedOk) {
        throw new Error("TradingView context_resolvers publish requires a reusable authenticated session. Refresh TV_STORAGE_STATE or TV_PERSISTENT_PROFILE_DIR first.");
      }
      ...
      if (!exactScriptVerified || !exactVersionVerified) {
        throw new Error(
          `Published TradingView context_resolvers library could not be verified exactly: live_script_verified=${publishedScriptVerified}, identity_mode=${identityVerificationMode}, version_mode=${versionVerificationMode}, expected_version=${details.version}, detected_version=${publishedVersion ?? "unknown"}, no_change_detected=${noChangeDetected}`,
        );
      }
    } finally {
    ...
    const report: PublishContextResolversReport = {
      generatedAt: utcNow(),
      ok: true,
      contractOk: true,
      publishAttempted,
