...
function parseArgs(): CliArgs {
  ...
  return {
    ...
    out: path.resolve(
      getFlag(
        "--out",
        `automation/tradingview/reports/publish-core-types-library-${utcNow().replace(/[:.]/g, "-")}.json`,
      ),
    ),
...
function verifyCoreTypesPublishContract(cli: CliArgs): ContractDetails {
  if (!fs.existsSync(cli.library)) {
    throw new Error(`Missing core_types library source: ${cli.library}`);
  }
  ...
  if (!libraryText.includes(expectedLibraryHeader)) {
    throw new Error(`Core types library header mismatch: expected ${expectedLibraryHeader}`);
  }
  ...
  if (!Number.isFinite(cli.version) || cli.version < 1) {
    throw new Error(`Core types library version must be a positive integer, received: ${cli.version}`);
  }
...
export async function runPublishCoreTypesLibraryCli(): Promise<number> {
  ...
  try {
    details = verifyCoreTypesPublishContract(cli);
    const session = await newTradingViewSession();
    try {
      if (!session.authResolution.authReusedOk) {
        throw new Error("TradingView core_types publish requires a reusable authenticated session. Refresh TV_STORAGE_STATE or TV_PERSISTENT_PROFILE_DIR first.");
      }
      ...
      if (!exactScriptVerified || !exactVersionVerified) {
        throw new Error(
          `Published TradingView core_types library could not be verified exactly: live_script_verified=${publishedScriptVerified}, identity_mode=${identityVerificationMode}, version_mode=${versionVerificationMode}, expected_version=${details.version}, detected_version=${publishedVersion ?? "unknown"}, no_change_detected=${noChangeDetected}`,
        );
      }
    } finally {
    ...
    const report: PublishCoreTypesReport = {
      generatedAt: utcNow(),
      ok: true,
      contractOk: true,
      publishAttempted,
