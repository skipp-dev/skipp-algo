"""Plan 2.8 ledger first unknown.

Reports the timestamp of the earliest ``unknown`` status
...
def render_markdown(report: dict[str, Any]) -> str:
    if not report.get("found"):
        return "# Plan 2.8 ledger first unknown\n\n_none_\n"
    return (
        "# Plan 2.8 ledger first unknown\n"
        "\n"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Timestamp of the first unknown ledger record.",
    )
