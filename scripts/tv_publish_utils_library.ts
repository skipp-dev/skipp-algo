...
function parseArgs(): CliArgs {
  ...
  return {
    ...
    out: path.resolve(
      getFlag(
        "--out",
        `automation/tradingview/reports/publish-utils-library-${utcNow().replace(/[:.]/g, "-")}.json`,
      ),
    ),
...
function verifyUtilsPublishContract(cli: CliArgs): ContractDetails {
  if (!fs.existsSync(cli.library)) {
    throw new Error(`Missing utils library source: ${cli.library}`);
  }
  ...
  if (!libraryText.includes(expectedLibraryHeader)) {
    throw new Error(`Utils library header mismatch: expected ${expectedLibraryHeader}`);
  }
  ...
  if (!Number.isFinite(cli.version) || cli.version < 1) {
    throw new Error(`Utils library version must be a positive integer, received: ${cli.version}`);
  }
...
export async function runPublishUtilsLibraryCli(): Promise<number> {
  ...
  try {
    details = verifyUtilsPublishContract(cli);
    const session = await newTradingViewSession();
    try {
      if (!session.authResolution.authReusedOk) {
        throw new Error("TradingView utils publish requires a reusable authenticated session. Refresh TV_STORAGE_STATE or TV_PERSISTENT_PROFILE_DIR first.");
      }
      ...
      if (!exactScriptVerified || !exactVersionVerified) {
        throw new Error(
          `Published TradingView utils library could not be verified exactly: live_script_verified=${publishedScriptVerified}, identity_mode=${identityVerificationMode}, version_mode=${versionVerificationMode}, expected_version=${details.version}, detected_version=${publishedVersion ?? "unknown"}, no_change_detected=${noChangeDetected}`,
        );
      }
    } finally {
    ...
    const report: PublishUtilsReport = {
      generatedAt: utcNow(),
      ok: true,
      contractOk: true,
      publishAttempted,
