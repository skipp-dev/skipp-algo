...
def build(artifact_dir: Path) -> dict[str, Any]:
    ...
    n = sum(1 for p in files if p.suffix.lower() == ".lz")
    return {
        "schema_version":    1,
...
def render_markdown(report: dict[str, Any]) -> str:
    return (
        ...
        f"- lz_file_count: {report['lz_file_count']}\n"
    )


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="lz file count.")
