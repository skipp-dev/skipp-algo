"""Plan 2.8 ledger last amber.

Reports the timestamp of the most recent ``amber`` status
record in the ledger (by insertion order). If no amber
...
def render_markdown(report: dict[str, Any]) -> str:
    if not report.get("found"):
        return "# Plan 2.8 ledger last amber\n\n_none_\n"
    return (
        "# Plan 2.8 ledger last amber\n"
        "\n"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Timestamp of the most recent amber ledger record.",
    )
