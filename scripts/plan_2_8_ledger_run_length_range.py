"""Plan 2.8 ledger run-length range.

Reports ``max - min`` of consecutive status run lengths.
Empty ledger yields 0. Single run yields 0.
"""
...
def compute(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if cur_status is not None:
        lengths.append(cur_length)
    rng = (max(lengths) - min(lengths)) if lengths else 0
    return {
        "schema_version": 1,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Run-length range.")
