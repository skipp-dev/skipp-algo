"""Plan 2.8 ledger status-run max.

Reports the single longest consecutive status run. Ties
...
def render_markdown(report: dict[str, Any]) -> str:
    if not report.get("found"):
        return "# Plan 2.8 ledger status-run max\n\n_none_\n"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Longest consecutive status run.",
    )
