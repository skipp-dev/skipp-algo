...
type PublishProfileEngineReport = {
  generatedAt: string;
  ok: boolean;
  contractOk: boolean;
...
function parseArgs(): CliArgs {
  ...
  return {
    ...
    out: path.resolve(
      getFlag(
        "--out",
        `automation/tradingview/reports/publish-profile-engine-library-${utcNow().replace(/[:.]/g, "-")}.json`,
      ),
    ),
...
function verifyProfileEnginePublishContract(cli: CliArgs): ContractDetails {
  if (!fs.existsSync(cli.library)) {
    throw new Error(`Missing profile_engine library source: ${cli.library}`);
  }
  ...
  if (!libraryText.includes(expectedLibraryHeader)) {
    throw new Error(`Profile engine library header mismatch: expected ${expectedLibraryHeader}`);
  }
  ...
  if (!Number.isFinite(cli.version) || cli.version < 1) {
    throw new Error(`Profile engine library version must be a positive integer, received: ${cli.version}`);
  }
...
export async function runPublishProfileEngineLibraryCli(): Promise<number> {
  ...
  try {
    details = verifyProfileEnginePublishContract(cli);
    const session = await newTradingViewSession();
    try {
      if (!session.authResolution.authReusedOk) {
        throw new Error("TradingView profile_engine publish requires a reusable authenticated session. Refresh TV_STORAGE_STATE or TV_PERSISTENT_PROFILE_DIR first.");
      }
      ...
      if (!exactScriptVerified || !exactVersionVerified) {
        throw new Error(
          `Published TradingView profile_engine library could not be verified exactly: live_script_verified=${publishedScriptVerified}, identity_mode=${identityVerificationMode}, version_mode=${versionVerificationMode}, expected_version=${details.version}, detected_version=${publishedVersion ?? "unknown"}, no_change_detected=${noChangeDetected}`,
        );
      }
    } finally {
    ...
    const report: PublishProfileEngineReport = {
      generatedAt: utcNow(),
      ok: true,
      contractOk: true,
      publishAttempted,
