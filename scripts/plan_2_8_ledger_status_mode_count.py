"""Plan 2.8 ledger status mode count."""
...
def compute(records: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    most = Counter(statuses).most_common(1)[0][1]
    return {
        "schema_version":       1,
        "record_count":         len(statuses),
        "status_mode_count":    most,
    }
...
def render_markdown(report: dict[str, Any]) -> str:
    return (
        ...
        f"- status_mode_count: {report['status_mode_count']}\n"
    )


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Status mode count.")
