...
def build(artifact_dir: Path) -> dict[str, Any]:
    ...
    n = sum(1 for p in files if p.suffix.lower() == ".z")
    return {
        "schema_version":    1,
...
def render_markdown(report: dict[str, Any]) -> str:
    return (
        ...
        f"- z_file_count: {report['z_file_count']}\n"
    )


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="z file count.")
