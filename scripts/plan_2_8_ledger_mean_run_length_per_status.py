...
Reports the mean consecutive-same-status run length for
each of the four canonical statuses. Statuses that never
appear yield 0.0. Means rounded to two decimals.
"""
...
def compute(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "mean_by_status": means,
    }


def render_markdown(report: dict[str, Any]) -> str:
    lines = ["# Plan 2.8 ledger mean run length per status", ""]
    for s in VALID_STATUSES:
        lines.append(f"- {s}: {report['mean_by_status'][s]}")
    lines.append("")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Mean run length per status.",
    )
