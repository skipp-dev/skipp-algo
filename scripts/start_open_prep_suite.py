...
def _run_open_prep(repo_root: Path, python_exe: str) -> None:
    ...
    with out_file.open("w", encoding="utf-8") as fh:
        subprocess.run(  # trusted: python_exe -m hardcoded module argv
...
def _stop_existing_monitor() -> None:
    ...
    subprocess.run([pkill_exe, "-f", "streamlit.*streamlit_monitor.py"], check=False)  # trusted: hardcoded pkill argv resolved via shutil.which
...
def _start_streamlit(repo_root: Path, python_exe: str, port: int) -> int:
    ...
    with log_file.open("w", encoding="utf-8") as fh:
        proc = subprocess.Popen(  # trusted: streamlit binary derived from python_exe sibling, hardcoded argv
            streamlit_cmd,
            cwd=str(repo_root),
            env=env,
            stdout=fh,
