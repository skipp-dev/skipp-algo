"""Plan 2.8 ledger last unknown.

Reports the timestamp of the most recent ``unknown`` status
record in the ledger (by insertion order). If no unknown
...
def compute(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for rec in records:
        ...
        if status not in VALID_STATUSES or status != "unknown":
            continue
...
def render_markdown(report: dict[str, Any]) -> str:
    if not report.get("found"):
        return "# Plan 2.8 ledger last unknown\n\n_none_\n"
    return (
        "# Plan 2.8 ledger last unknown\n"
        "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Timestamp of the most recent unknown ledger record.",
    )
