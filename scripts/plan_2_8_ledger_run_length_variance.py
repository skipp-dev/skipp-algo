"""Plan 2.8 ledger run-length variance.

Reports the population variance of consecutive status run
lengths. Fewer than one run yields 0.0.
"""
...
def compute(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    n = len(lengths)
    if n == 0:
        return {"schema_version": 1, "run_count": 0, "variance": 0.0}
    mean = sum(lengths) / n
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Run-length variance.")
